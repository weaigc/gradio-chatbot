...
import { fetch, WebSocket, Blob } from "ifw";
...
export async function post_data(
	...
	const output = await response.json() as PostResponse;
...
export async function duplicate(
	...
	try {
		...
		if (response.status === 409) {
			...
			const duplicated_space = await response.json() as any;
			...
			handle_blob(
				...
				if (skip_queue(fn_index, config)) {
					...
					websocket.onmessage = function (event: any) {
						const _data = JSON.parse(event.data as any);
	...
	if (Array.isArray(param)) {
		...
		if (type === "Image") {
			...
			if (typeof window !== "undefined") {
				// browser
				data = await image_to_data_uri(param as any);
			} else {
